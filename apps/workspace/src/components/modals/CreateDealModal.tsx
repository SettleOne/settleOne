...
import {
  X, ChevronRight, ChevronLeft, FileText, Wallet, ShieldCheck, Check,
  Code, Package, AlertCircle, Upload, Trash2
} from "lucide-react";
...
const SOFTWARE_CATEGORIES = [
  "All", "Smart Contract Audit", "Web Development", "Mobile App",
  "Design/UI", "Backend/API", "Data/Analytics", "Consulting", "Other"
];
const HARDWARE_CATEGORIES = [
  "All", "Electronics", "Manufacturing", "Equipment",
  "Furniture", "Wearables", "Inventory", "Other"
];
...
export function CreateDealModal({ isOpen, onClose }: CreateDealModalProps) {
  ...
  const categories = form.dealType === "software" ? SOFTWARE_CATEGORIES : HARDWARE_CATEGORIES;
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[var(--border)] shrink-0">
          <div>
            <h2 className="text-xl font-bold text-[var(--text-primary)]">Create a New Deal</h2>
              ...
              <React.Fragment key={s.num}>
                ...
                {i < steps.length - 1 && (
                  <div
                    ...
                    style={{ background: step > s.num ? "var(--accent-blue)" : "var(--border)" }}
                  />
                )}
              </React.Fragment>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          {isSuccess ? (
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Deal Created!</h3>
              <p className="text-[var(--text-secondary)] mb-1">Deal ID: <span className="font-mono text-[var(--text-primary)]">#{createdId}</span></p>
              <p className="text-xs text-[var(--text-muted)] mb-6">
                Share: settleone.xyz/marketplace/{createdId}
              </p>
              <div className="flex gap-3 justify-center">
                <button
                  onClick={() => { onClose(); navigate(`/marketplace/${createdId.replace("DL-", "")}`); }}
            ...
            <>
              ...
              <div>
                <div className="flex justify-between mb-1.5">
                  <label className="text-sm font-medium text-[var(--text-secondary)]">Deal Name <span className="text-[var(--accent-red)]">*</span></label>
                  <span className={`text-xs ${charCount > 100 ? "text-[var(--accent-red)]" : "text-[var(--text-muted)]"}`}>{charCount}/120</span>
                </div>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(["software", "hardware"] as const).map((type) => (
                    ...
                    >
                      <div className={`mb-2 ${form.dealType === type ? "text-[var(--accent-blue)]" : "text-[var(--text-muted)]"}`}>
                        {type === "software" ? <Code size={20} /> : <Package size={20} />}
                      </div>
                      <p className="font-semibold text-sm text-[var(--text-primary)] capitalize">
                        {type === "software" ? "Software / Digital" : "Hardware / Physical"}
                      </p>
                      <p className="text-xs text-[var(--text-muted)] mt-0.5">
                        {type === "software" ? "Code, designs, audits, consulting" : "Products, equipment, inventory"}
                      </p>
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Deal Description <span className="text-[var(--accent-red)]">*</span>
                </label>
                <textarea
                ...
                {form.description.length > 0 && form.description.length < 100 && (
                  <p className="text-xs text-[var(--accent-amber)] mt-1 flex items-center gap-1">
                    <AlertCircle size={12} /> Minimum 100 characters ({form.description.length}/100)
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                    Deal Amount <span className="text-[var(--accent-red)]">*</span>
                  </label>
                  <input
                    type="number"
                    value={form.amount}
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Token</label>
                  <select
                    value={form.token}
              ...
              {/* Chain */}
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Chain</label>
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Seller Address <span className="text-[var(--text-muted)] font-normal">(optional)</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">
                  Funding Option <span className="text-[var(--accent-red)]">*</span>
                </label>
                <div className="space-y-2">
                  {[
                    { key: "full", label: "Full payment upfront (100%)", desc: "Pay everything at funding stage" },
                    { key: "staged", label: "Staged funding: 60% now, 40% before acceptance", desc: "Seller knows remaining 40% is coming only if delivery passes verification" },
                  ].map((opt) => (
                    <label
                      key={opt.key}
              ...
              <div className="flex items-center justify-between p-3 bg-[var(--bg-base)] border border-[var(--border)] rounded-[var(--radius-input)]">
                <div>
                  <p className="text-sm font-medium text-[var(--text-primary)]">Partial Settlement Allowed</p>
                  <p className="text-xs text-[var(--text-muted)]">Allow arbitrator to split the amount in disputes</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) => update("partialSettlement", e.target.checked)}
                  />
            ...
            <>
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Seller Acceptance Window <span className="text-[var(--accent-red)]">*</span>
                </label>
                <select
                ...
                <p className="text-xs text-[var(--text-muted)] mt-1">After you fund, the seller acceptance countdown starts.</p>
              </div>

              {/* Delivery Deadline */}
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Delivery Deadline <span className="text-[var(--accent-red)]">*</span>
                </label>
                <input
                  type="date"
                ...
                />
                <p className="text-xs text-[var(--text-muted)] mt-1">Seller must submit verified delivery by this date or you are automatically refunded.</p>
              </div>

              {/* Buyer Acceptance Window */}
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Buyer Acceptance Window <span className="text-[var(--accent-red)]">*</span>
                </label>
                <select
                ...
                <p className="text-xs text-[var(--text-muted)] mt-1">If you don't act, delivery is auto-accepted after this window.</p>
              </div>

              {/* Dispute Window */}
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Dispute Window <span className="text-[var(--accent-red)]">*</span>
                </label>
                <select
                ...
                <p className="text-xs text-[var(--text-muted)] mt-1">After you accept, this is your window to raise a dispute.</p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    { key: "protocol", label: "Protocol Default Verifier", desc: "SettleOne's EIP-712 validator (recommended)" },
                    { key: "chainlink", label: "Chainlink Oracle Verifier", desc: "Async oracle node verifies delivery independently" },
                    { key: "custom", label: "Custom Address (advanced)", desc: "Specify your own verifier contract" },
                  ].map((opt) => (
                    <label
                      key={opt.key}
                      className={`flex items-start gap-3 p-3 rounded-[var(--radius-input)] border cursor-pointer transition-all ${
                    ...
                    >
                      <input type="radio" checked={form.verifier === opt.key} onChange={() => update("verifier", opt.key)} className="mt-1 accent-[var(--accent-blue)]" />
                      <div>
                        <p className="text-sm font-medium text-[var(--text-primary)]">{opt.label}</p>
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">
                  Dispute Resolver <span className="text-[var(--accent-red)]">*</span>
                </label>
                <div className="space-y-2">
                  {[
                    { key: "protocol", label: "Protocol Default Arbitrator", desc: "SettleOne's qualified arbitration panel (recommended)" },
                    { key: "custom", label: "Custom Address (advanced)", desc: "Specify your own resolver contract" },
                  ].map((opt) => (
                    <label
                      key={opt.key}
                      className={`flex items-start gap-3 p-3 rounded-[var(--radius-input)] border cursor-pointer transition-all ${
                    ...
                    >
                      <input type="radio" checked={form.resolver === opt.key} onChange={() => update("resolver", opt.key)} className="mt-1 accent-[var(--accent-blue)]" />
                      <div>
                        <p className="text-sm font-medium text-[var(--text-primary)]">{opt.label}</p>
                        <p className="text-xs text-[var(--text-muted)]">{opt.desc}</p>
                      </div>
            ...
            <>
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">
                  Terms & Commercial Agreement <span className="text-[var(--text-muted)] font-normal">(optional)</span>
                </label>
                <label
                  className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-[var(--border)] rounded-[var(--radius-card)] cursor-pointer hover:border-[var(--accent-blue)] hover:bg-[var(--accent-blue-glow2)] transition-all"
                >
                  <Upload size={24} className="text-[var(--text-muted)] mb-2" />
                  <span className="text-sm text-[var(--text-secondary)]">
                    {form.termsFile ? form.termsFile.name : "Drag & drop or click to upload"}
                  </span>
                  <span className="text-xs text-[var(--text-muted)] mt-1">PDF, DOC, TXT — max 10MB</span>
                  <input
                    type="file"
                    className="hidden"
                    accept=".pdf,.doc,.docx,.txt"
                    onChange={(e) => update("termsFile", e.target.files?.[0] || null)}
                  />
                </label>
                {form.termsFile && (
                  <div className="flex items-center gap-2 mt-2 px-3 py-2 bg-[var(--bg-base)] border border-[var(--border)] rounded-[var(--radius-input)] text-sm">
                    ...
                    <span className="text-[var(--text-primary)] flex-1 truncate">{form.termsFile.name}</span>
                    <button onClick={() => update("termsFile", null)} className="text-[var(--text-muted)] hover:text-[var(--accent-red)]">
                      <Trash2 size={14} />
                    </button>
                  </div>
                )}
                <p className="text-xs text-[var(--text-muted)] mt-1">
                  This document is hashed and its hash committed on-chain as termsHash. The file itself is stored encrypted off-chain.
                </p>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Evidence Requirements <span className="text-[var(--text-muted)] font-normal">(optional)</span>
                </label>
                <textarea
                  value={form.evidenceRequirements}
                  onChange={(e) => update("evidenceRequirements", e.target.value)}
                  rows={3}
                ...
                />
                <p className="text-xs text-[var(--text-muted)] mt-1">This is hashed as evidenceRequirementsHash.</p>
              </div>

              {/* Settlement Rules */}
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                  Settlement Rules <span className="text-[var(--text-muted)] font-normal">(optional)</span>
                </label>
                <textarea
              ...
              {/* Preview Panel */}
              <div className="p-4 bg-[var(--bg-base)] border border-[var(--border)] rounded-[var(--radius-card)]">
                <h4 className="text-sm font-semibold text-[var(--text-secondary)] mb-3 uppercase tracking-wider">Deal Preview</h4>
                <div className="space-y-2 text-sm font-mono">
                  {[
                    ...
                    ["Chain", form.chain],
                    ["Funding", form.fundingOption === "staged" ? "60/40 staged" : "100% upfront"],
              ...
              {/* Disclaimer */}
              <div className="flex items-start gap-2 p-3 bg-[var(--accent-amber)]/10 border border-[var(--accent-amber)]/30 rounded-[var(--radius-input)]">
                <AlertCircle size={16} className="text-[var(--accent-amber)] mt-0.5 shrink-0" />
                <p className="text-xs text-[var(--text-secondary)]">
                  Creating this deal will call <span className="font-mono text-[var(--text-primary)]">DealManager.createDeal()</span> on {form.chain}.

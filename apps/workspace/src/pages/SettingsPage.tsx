...
import { Bell, Wallet, Shield, AlertTriangle, Download, Trash2, CheckCircle2 } from "lucide-react";
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-card)] p-8 shadow-[var(--shadow-card)]">
            {activeTab === "notifications" && (
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-6">
                    {[
                      { title: "Deal State Changes", desc: "When a deal is accepted, funded, or settled." },
                      { title: "Deadline Reminders", desc: "Warnings when delivery or acceptance windows are closing." },
                      { title: "Dispute Updates", desc: "Alerts when a dispute is raised or resolved." },
                      { title: "New Messages", desc: "When you receive a new message in the inbox." },
                    ].map((item, i) => (
                      <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between py-4 border-b border-[var(--border)] last:border-0 gap-4">
                        <div>
                        ...
                        <div className="flex gap-6 bg-[var(--bg-base)] p-3 rounded-[var(--radius-input)] border border-[var(--border)]">
                          <label className="flex items-center gap-2 cursor-pointer text-sm font-medium hover:text-[var(--accent-blue-bright)] transition-colors">
                            <input type="checkbox" className="accent-[var(--accent-blue)] w-4 h-4 cursor-pointer" defaultChecked />
                            Email
                          </label>
                          <label className="flex items-center gap-2 cursor-pointer text-sm font-medium hover:text-[var(--accent-blue-bright)] transition-colors">
                            <input type="checkbox" className="accent-[var(--accent-blue)] w-4 h-4 cursor-pointer" defaultChecked />
            ...
            {activeTab === "wallet" && (
              <div className="space-y-6">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Wallet className="text-[var(--accent-blue)]" /> Wallet & Network</h2>
                <div className="p-5 bg-[var(--bg-subtle)] border border-[var(--border)] rounded-[var(--radius-input)] flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <p className="font-mono text-base font-medium">0x1234...5678</p>
                    </div>
                  </div>
                  <Button variant="secondary" className="hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/50">Disconnect</Button>
                </div>
                <div className="space-y-5 mt-8">
                  <div>
                    <label className="block text-sm font-medium mb-2 text-[var(--text-secondary)]">Preferred Network</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2 text-[var(--text-secondary)]">Gas Limit Preferences</label>
                    <select className="w-full bg-[var(--bg-base)] border border-[var(--border)] rounded-[var(--radius-input)] px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-blue)] focus:ring-1 focus:ring-[var(--accent-blue)] transition-all">
            ...
            {activeTab === "privacy" && (
              <div className="space-y-6">
                ...
                <div className="space-y-8 mt-6">
                  <div>
                    <label className="block text-sm font-medium mb-2 text-[var(--text-secondary)]">Profile Visibility</label>
                    <select className="w-full bg-[var(--bg-base)] border border-[var(--border)] rounded-[var(--radius-input)] px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-blue)] focus:ring-1 focus:ring-[var(--accent-blue)] transition-all">
                  ...
                  <div className="flex items-center justify-between p-4 bg-[var(--bg-subtle)] border border-[var(--border)] rounded-[var(--radius-input)]">
                    <div>
                      ...
                      <p className="text-[var(--text-muted)] text-sm mt-1">
                        If disabled, your address will only be visible to parties in active deals.
                      </p>
                    </div>
            ...
            {activeTab === "danger" && (
              <div className="space-y-6">
                <h2 className="text-xl font-bold mb-4 text-[var(--accent-red)] flex items-center gap-2"><AlertTriangle /> Danger Zone</h2>
                  ...
                  <div className="p-5 border-b border-[var(--accent-red)]/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-[var(--text-secondary)] text-sm mt-1">
                        Download a JSON file containing all your deals and account history.
                      </p>
                    </div>

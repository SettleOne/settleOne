...
export function DealCard({
  ...
  return (
    <Link
      to={`/marketplace/${id}`}
      className="block group relative overflow-hidden rounded-[var(--radius-card)]"
    >
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-card)] p-5 flex flex-col h-full hover:border-[var(--accent-blue)] hover:shadow-[var(--shadow-glow)] transition-all relative z-10 duration-300">
        ...
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-2">
            <span
              className={`text-xs font-bold px-2 py-1 rounded-[var(--radius-pill)] ${
                isSoftware
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[var(--bg-subtle)] border border-[var(--border)]">
              <div
                ...
                style={{
                  backgroundColor: getDealStateColor(state),
                  boxShadow: `0 0 8px ${getDealStateColor(state)}`,
                }}
              />
              <span
        ...
        <div className="mb-4">
          ...
          {isStagedFunding && (
            <p className="text-xs text-[var(--text-secondary)] mt-1">
              Deposited: {formatAmount(depositedAmount, 6)} {tokenSymbol} (
              {fundingPercentage}%)
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2 mb-4">
          ...
          {deliveryDeadline > 0 &&
            Number(deliveryDeadline) * 1000 - Date.now() <
              3 * 24 * 60 * 60 * 1000 && (
          ...
          <div className="h-1 w-full bg-[var(--bg-base)] rounded-full mb-4 overflow-hidden border border-[var(--border)]">
            <div
              ...
              style={{
                ...
                boxShadow: `0 0 10px ${fundingPercentage === 100 ? "var(--accent-green)" : "var(--accent-blue)"}`,
              }}
            />
          </div>
        )}
        ...
        <div className="mt-auto pt-4 flex items-center justify-between border-t border-[var(--border)]">
          <div className="flex items-center gap-2">
            <NetworkBadge
              chainId={chainId}
              className="!py-1 !px-2 !bg-[var(--bg-subtle)] !border-[var(--border)] !text-[var(--text-primary)]"
            />

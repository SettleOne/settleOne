...
function AddressRow({ label, address, user, empty }: { label: string; address?: string | null; user?: any; empty?: string }) {
    ...
    <div className="py-2.5 border-b border-[var(--border)] last:border-0 flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <span className="text-xs font-mono text-[var(--text-secondary)]">{short}</span>
      ...
      {user && (
        <div className="flex items-center gap-2 mt-0.5 bg-[var(--bg-base)] p-1.5 rounded-md border border-[var(--border)]">
          {user.avatarUrl ? (
            <img src={user.avatarUrl} alt={user.name || "User"} className="w-6 h-6 rounded-full object-cover shrink-0" />
          ) : (
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold text-[var(--text-primary)] truncate">{user.name || "Unknown User"}</p>
            {user.organization && <p className="text-[10px] text-[var(--text-muted)] truncate">{user.organization}</p>}
          </div>
        </div>
      )}
    </div>
...
export function DealInfoSidebar({ deal }: { deal: any }) {
  const chainName = Object.keys(CHAIN_CONFIG).find((key: any) => CHAIN_CONFIG[key as keyof typeof CHAIN_CONFIG].chainId === deal.chainId) || "Unknown";
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-4 space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-[var(--text-primary)] capitalize">{deal.fundingType || "full"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="px-4 pb-2">
          <AddressRow label="Buyer" address={deal.buyerAddress} user={deal.buyer} />
          <AddressRow label="Seller" address={deal.sellerAddress} user={deal.seller} />
          <AddressRow label="Verifier" address={deal.verifierAddress} user={deal.verifier} />
          <AddressRow label="Resolver" address={deal.resolverAddress} user={deal.resolver} />
        </div>
      </div>
        ...
        <div className="p-4 space-y-3 text-sm">
          ...
          {deal.acceptanceWindowEndsAt && (
            <div className="flex justify-between">
              ...
              <span className="font-medium text-[var(--accent-amber)]">{new Date(deal.acceptanceWindowEndsAt).toLocaleDateString()}</span>
            </div>
          )}
          {deal.disputeWindowEndsAt && (
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">Dispute Ends</span>
              <span className="font-medium text-[var(--accent-red)]">{new Date(deal.disputeWindowEndsAt).toLocaleDateString()}</span>
            </div>
          )}
        </div>
      </div>

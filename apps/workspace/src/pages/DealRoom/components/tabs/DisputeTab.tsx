...
export function DisputeTab({ deal }: { deal: any }) {
  const activeDispute =
    deal?.disputes?.find((d: any) => d.status !== "Cancelled") ||
    deal?.disputes?.[0];
    ...
    return (
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-8 shadow-sm flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm text-[var(--text-secondary)] max-w-md">
          This deal is proceeding smoothly. No disputes have been filed by
          either party.
        </p>
      </div>
    );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-[var(--text-muted)] flex items-center gap-1">
              ...
              Opened:{" "}
              {new Date(
                activeDispute.openedAt || activeDispute.createdAt,
              ).toLocaleDateString()}
            </span>
            <span
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <p className="text-[var(--text-primary)] font-mono text-sm break-all">
              {activeDispute.openerAddress || "Unknown"}
            </p>
          </div>
          <div className="md:col-span-2">
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <div className="bg-[var(--bg-subtle)] p-3 rounded-lg border border-[var(--border)]">
              ...
              <span className="font-semibold text-[var(--text-primary)]">
                {verdict.sellerAward?.toString() || "0"}
              </span>
            </div>
            <div className="bg-[var(--bg-subtle)] p-3 rounded-lg border border-[var(--border)]">
              <span className="text-xs text-[var(--text-muted)] block mb-1">
            ...
            <div className="bg-[var(--bg-subtle)] p-3 rounded-lg border border-[var(--border)]">
              ...
              <span
                className="font-semibold text-[var(--text-primary)] font-mono truncate block"
                title={verdict.resolutionHash}
              >
                {verdict.resolutionHash
                  ? verdict.resolutionHash.slice(0, 10) + "..."
                  : "None"}
              </span>
            </div>
          </div>

  ...
  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 shadow-sm">
      ...
      <p className="text-sm text-[var(--text-secondary)] mb-6">
        All encrypted files and evidence associated with this deal are stored
        here.
      </p>
      ...
      {!deal?.files || deal.files.length === 0 ? (
        ...
        <div className="space-y-4">
          {deal.files.map((file: any, i: number) => (
            <div
              key={i}
              className="flex flex-col xl:flex-row justify-between items-start xl:items-center p-4 bg-[var(--bg-subtle)] border border-[var(--border)] rounded-lg gap-4"
            >
              ...
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                <span
                  className="text-xs px-2 py-1 bg-[var(--bg-base)] border border-[var(--border)] text-[var(--text-secondary)] rounded flex items-center gap-1 capitalize"
                  title="Access Level"
                >
                  <Lock size={12} />
                ...
                {file.ipfsCid && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs px-2 py-1 bg-[var(--accent-purple)]/10 text-[var(--accent-purple)] rounded flex items-center gap-1 hover:bg-[var(--accent-purple)]/20 transition-colors"
                  >
                    <Globe size={12} />
                    IPFS
                  </a>
                ...
                {file.isCommittedOnchain && (
                  <span
                    className="text-xs px-2 py-1 bg-[var(--accent-green)]/10 text-[var(--accent-green)] rounded flex items-center gap-1"
                    title={`Evidence ID: ${file.onChainEvidenceId}`}
                  >
                    <ShieldCheck size={12} />
                    On-chain
                  </span>

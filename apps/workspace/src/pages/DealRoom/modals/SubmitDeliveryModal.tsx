...
interface SubmitDeliveryModalProps {
  isOpen: boolean;
  onClose: () => void;
  dealId: string; // UUID for API
  onChainId: bigint; // for Contract
}
...
export function SubmitDeliveryModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      setStatus("uploading");
      // 1. Upload all files via backend upload API
      ...
      // 2. Submit metadata to off-chain API to get the correct proofHash
      const response = await submitOffChain({
        dealId,
        fileIds,
        sellerNotes: notes,
      });
      ...
      // 3. Submit on-chain using the exact hash generated by the backend
      const proofHash = response.data?.proofHash;
      await submitDeliveryProof(onChainId, proofHash as `0x${string}`);
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <FileUploadZone
                ...
                maxFiles={5}
                maxSizeMB={50}
                label="Drag and drop delivery files or zip archives"
              />
              {files.length > 0 && (
                <div className="mt-3 space-y-2 max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                  {files.map((file, idx) => (
                    <div
                      key={idx}
                      className="p-2.5 bg-[var(--bg-subtle)] rounded-md border border-[var(--border)] flex items-center justify-between"
                    >
                      <span className="text-xs font-medium text-[var(--text-primary)] truncate max-w-[70%]">
                        {file.name}
                      </span>

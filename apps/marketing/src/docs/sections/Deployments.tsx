...
import { PageHeader, Section, Paragraph, ContractCard, Callout, List } from "../components/DocsUI";

export function ContractAddresses() {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader title="Contract Addresses" description="Publicly verifiable SettleOne protocol deployments." />
      
      <Callout type="info">
        Only verified deployment addresses should be published. The addresses listed below are the official SettleOne deployments.
      </Callout>
...
export function SecurityModel() {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader title="Security Model" description="Defense-in-depth architecture and risk awareness." />
      <Section title="Multi-Layered Security">
        <Paragraph>SettleOne utilizes a defense-in-depth security approach. Smart contracts enforce critical financial constraints, ensuring that backend services do not replace on-chain security.</Paragraph>
        <List>
          ...
          <li><strong>External protocol risk remains visible:</strong> SettleOne provides transparency regarding where and how capital is deployed.</li>
        </List>
      </Section>
      <Section title="Risk Model">
        <Paragraph>SettleOne does not eliminate DeFi risk. Participants should be aware of potential risks, including smart-contract risk, underlying protocol risk (e.g. Aave), market risk, and liquidity constraints.</Paragraph>
        <Callout type="warning" title="No Guarantees">
          Risk policies reduce exposure; they do not make loss impossible. SettleOne does not guarantee principal preservation or yield unless an explicit protection mechanism exists in the deal's terms.
        </Callout>
      </Section>
    </div>
  );

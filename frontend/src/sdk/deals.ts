...
export const dealsSdk = {
  createDeal: async (walletClient: any, publicClient: any, input: DealInput) => {
  ...
  depositFunds: async (walletClient: any, publicClient: any, dealId: bigint, amount: string) => {
    const { request } = await publicClient.simulateContract({
      address: DEAL_MANAGER_ADDRESS,
      abi: DEAL_MANAGER_ABI,
  ...
  submitProof: async (walletClient: any, publicClient: any, dealId: bigint, proofHash: `0x${string}`) => {
    const { request } = await publicClient.simulateContract({
      address: DEAL_MANAGER_ADDRESS,
      abi: DEAL_MANAGER_ABI,
  ...
  confirmDelivery: async (walletClient: any, publicClient: any, dealId: bigint) => {
    const { request } = await publicClient.simulateContract({
      address: DEAL_MANAGER_ADDRESS,
      abi: DEAL_MANAGER_ABI,

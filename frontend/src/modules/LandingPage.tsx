...
import { 
  ...
  Lock, CheckCircle, Gavel, Globe, Cpu 
} from 'lucide-react'
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-bg-primary">
      ...
      <AnimatePresence mode="wait">
        {scene === 0 && (
          ...
          >
            ...
            >
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-brand-teal">Protocol v1.0 Live on Sepolia</span>
            </motion.div>
            ...
            >
              You deliver first.<br />
              Then <span className="opacity-10 italic">wait</span>, <span className="opacity-10 italic">chase</span>,<br />
              <span className="text-brand-teal not-italic font-syne font-black text-glow-teal">hope</span> to get paid.
            </motion.h1>
            ...
            >
              <span className="hover:text-white transition-colors cursor-default">Delayed</span>
              <span className="text-text-muted opacity-30">•</span>
              <span className="text-brand-gold hover:text-brand-gold-dim transition-colors cursor-default">Disputed</span>
              <span className="text-text-muted opacity-30">•</span>
              <span className="hover:text-white transition-colors cursor-default">Uncertain</span>
            </motion.div>
          </motion.div>
        )}
        ...
        {scene === 1 && (
          ...
          >
            <div className="text-center mb-16">
              <span className="font-mono text-[10px] uppercase tracking-[0.5em] text-text-muted">THE SETTLEONE ARCHITECTURE</span>
            </div>
            
            <div className="grid gap-6">
              {[
                { num: '01', word: 'COMMIT', detail: 'Capital secured in immutable vault before workflow initiation.', sub: 'BUYER → ESCROW', icon: Lock, color: 'text-brand-teal' },
                { num: '02', word: 'VERIFY', detail: 'Onchain delivery manifest hashed and validated by network nodes.', sub: 'PROOF → CONTRACT', icon: Globe, color: 'text-brand-teal' },
                { num: '03', word: 'SETTLE', detail: 'Atomic distribution of assets upon protocol satisfaction.', sub: 'CONTRACT → RELEASE', icon: Zap, color: 'text-brand-gold' },
              ].map((item, i) => (
                <motion.div 
                  key={item.num}
                ...
                >
                  ...
                  <div className="flex items-center gap-12 relative z-10">
                    <span className="font-mono text-xs text-text-muted opacity-50">{item.num}</span>
                    <div className="space-y-1">
                      <h2 className={cn("font-syne font-black text-6xl md:text-8xl tracking-tighter leading-none", item.color)}>
                        {item.word}
                      </h2>
                      <p className={cn("font-mono text-[10px] tracking-[0.3em] uppercase ml-1 opacity-70", item.color === 'text-brand-gold' ? 'text-brand-gold' : 'text-brand-teal')}>
                        {item.sub}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-right max-w-xs relative z-10 hidden md:block">
                    <item.icon className={cn("w-8 h-8 ml-auto mb-4 opacity-20 group-hover:opacity-100 transition-all duration-700", item.color)} />
        ...
        {scene === 2 && (
          ...
          >
            ...
            >
              The first <span className="text-brand-teal font-bold px-1">onchain</span> payment commitment layer. 
              Secure <span className="text-white">business obligations</span> and service settlements before work begins.
            </motion.p>
      ...
      <div className="absolute bottom-10 left-0 right-0 z-10 flex items-center justify-center gap-16 opacity-20 grayscale hover:opacity-100 hover:grayscale-0 transition-all duration-1000">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-[9px] uppercase tracking-[0.3em]">Network Active</span>
        </div>
        <div className="flex items-center gap-3">
          <ShieldCheck className="w-4 h-4 text-brand-teal" />
          <span className="font-mono text-[9px] uppercase tracking-[0.3em]">Audited Vaults</span>
        </div>
        <div className="flex items-center gap-3">
          <CheckCircle className="w-4 h-4 text-brand-gold" />
          <span className="font-mono text-[9px] uppercase tracking-[0.3em]">100% Settlement Rate</span>
        </div>
      </div>
    </div>
  )

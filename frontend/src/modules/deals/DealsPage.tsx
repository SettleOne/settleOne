...
import { 
  ...
  Loader2, Wallet, TrendingUp, Inbox,
  ExternalLink, Clock
} from 'lucide-react'
...
export function DealsPage() {
  ...
  const filteredDeals = deals.filter(deal => {
    ...
    const matchesFilter = filter === 'ALL' || deal.status.toUpperCase() === filter
    ...
    <div className="container max-w-7xl mx-auto px-4 py-12 relative z-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
        <div className="space-y-2">
          <h1 className="font-syne font-extrabold text-4xl md:text-5xl tracking-tight">
            Protocol <span className="text-brand-teal text-glow-teal">Inventory</span>
          </h1>
          <p className="text-text-slate font-mono text-xs uppercase tracking-[0.3em]">Monitor and manage active payment commitments</p>
        </div>
        <Link to="/deals/create">
          <Button size="lg" className="h-14 px-8 gap-3 font-syne font-bold shadow-lg">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 gap-6">
          ...
          <p className="font-mono text-[10px] uppercase tracking-[0.4em] text-text-slate animate-pulse">Syncing with Mainnet</p>
        </div>
        ...
        <Card className="py-24 text-center">
          ...
          <h3 className="text-xl font-syne font-bold mb-2">No Protocols Detected</h3>
          <p className="text-text-slate max-w-sm mx-auto text-sm">
            We couldn't find any protocol instances matching your current filter. Initialize a new commitment to get started.
          </p>
        </Card>
      ) : (
                ...
                <Card className="h-full flex flex-col hover:border-brand-teal/30 transition-all duration-500 hover:shadow-2xl hover:shadow-brand-teal/5 group">
                  <CardHeader className="space-y-4">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-1.5 text-text-slate">
                        ...
                        <span className="text-[9px] font-mono uppercase tracking-widest">{new Date(deal.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                    <div>
                      <CardTitle className="line-clamp-1 group-hover:text-brand-teal transition-colors">{deal.title}</CardTitle>
                      <div className="flex items-center gap-2 mt-2">
                        <Wallet className="w-3 h-3 text-text-slate" />
                        <CardDescription className="normal-case tracking-tight truncate max-w-[200px]">{deal.buyerWallet}</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  ...
                  <CardContent className="flex-1 space-y-6">
                    <div className="p-4 rounded-xl bg-bg-tertiary/50 border border-text-muted/5 group-hover:border-brand-teal/10 transition-all">
                      <p className="text-[9px] font-mono text-text-slate uppercase tracking-[0.2em] mb-1">Obligation</p>
                      <p className="text-2xl font-syne font-black text-brand-teal tracking-tighter">
                        {formatEther(deal.amountWei)} <span className="text-xs font-normal opacity-50">{deal.currencySymbol}</span>
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="h-1 w-full bg-bg-tertiary rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${deal.status === 'settled' ? '100' : '45'}%` }}
                        />
                      </div>
                    </div>
                  </CardContent>
                  ...
                  <CardFooter className="pt-2">
                    <Link to={`/deals/${deal.contractDealId || deal.id}`} className="w-full">
                      <Button variant="outline" className="w-full justify-between h-12 hover:bg-brand-teal/10 group-hover:border-brand-teal/30">
                        View Manifest
      ...
      <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-8 p-8 glass-card border-brand-teal/5">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-mono text-text-slate uppercase tracking-widest">Protocol Throughput</p>
        ...
        <div className="flex items-center gap-4 border-l border-text-muted/10 pl-8">
          ...
          <div>
            <p className="text-[10px] font-mono text-text-slate uppercase tracking-widest">Trust Integrity</p>
        ...
        <div className="flex items-center gap-4 border-l border-text-muted/10 pl-8">
          ...
          <div>
            <p className="text-[10px] font-mono text-text-slate uppercase tracking-widest">Network Nodes</p>

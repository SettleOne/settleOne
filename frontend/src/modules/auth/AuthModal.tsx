...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
      // 1. Get nonce
      const { data: nonceData } = await apiClient.post('/users/nonce', { walletAddress: address })
          ...
          >
            <div className="p-4 rounded-xl bg-brand-teal/5 border border-brand-teal/10 flex items-start gap-4">
              ...
              <div className="text-sm">
                <p className="font-bold text-text-primary">Secure Authentication</p>
                <p className="text-text-slate mt-1 leading-relaxed">
                  We use Sign-In with Ethereum to securely verify your identity without passwords.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-lg bg-bg-tertiary border border-text-muted/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-mono uppercase tracking-widest text-text-slate">Connected Wallet</p>
          ...
          >
            <form onSubmit={handleSubmit(handleRegister)} className="space-y-5">
              <p className="text-sm text-text-slate mb-6">
                It looks like this is your first time here. Let's get your profile set up.
              </p>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-mono uppercase tracking-widest text-text-slate ml-1">Display Name</label>
                <input 
                  {...register('displayName')}
                  className={cn(
                    "w-full bg-bg-tertiary border rounded-lg px-4 py-3 focus:outline-none focus:border-brand-teal transition-all",
                    errors.displayName ? "border-red-500/50" : "border-text-muted/20"
                  )}
                  placeholder="How should we call you?"
                />
                {errors.displayName && <p className="text-red-500 text-[10px] mt-1 ml-1">{errors.displayName.message}</p>}
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-mono uppercase tracking-widest text-text-slate ml-1">Email Address</label>
                <input 
                  {...register('email')}
                  type="email"
                  className={cn(
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-mono uppercase tracking-widest text-text-slate ml-1">Preferred Role</label>
                  ...
                  {['buyer', 'seller'].map((role) => (
                    <label 
                      ...
                      className={cn(
                        ...
                        // @ts-ignore
                        register('role').value === role ? "bg-brand-teal/10 border-brand-teal text-brand-teal" : "bg-bg-tertiary border-text-muted/10 text-text-slate hover:border-text-muted/30"
                      )}
                    >
                      <input 
                        type="radio" 
              ...
              <Button type="submit" className="w-full h-14 text-base font-syne font-bold mt-4">

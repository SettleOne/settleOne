...
export function NotificationsPage() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case "warning":
        return <AlertTriangle className="w-5 h-5 text-brand-gold" />;
      case "error":
          ...
          {notifications.map((n) => (
            <Card
              ...
              className={cn(
                ...
                !n.read
                  ? "border-brand-teal/30 bg-brand-teal/[0.02]"
                  : "opacity-80",
              )}
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex justify-between items-start">
                    <h4
                      className={cn(
                        "font-bold text-lg",
                        !n.read ? "text-text-primary" : "text-text-slate",
                      )}
                    >
                      {n.title}
                    </h4>
                  ...
                  {!n.read && (
                    <div className="pt-2">
                      ...
                      <span className="ml-2 text-[10px] font-mono text-brand-teal uppercase tracking-widest">
                        New Transmission
                      </span>
                    </div>
                  )}

...
export function FileUpload({ onUploadComplete, onCancel }: FileUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const mockHash = `0x${Array.from(new Uint8Array(32)).map(() => Math.floor(Math.random() * 16).toString(16)).join('')}` as `0x${string}`
    ...
    <div className="space-y-6">
      <div 
        className={cn(
          ...
          file ? "border-brand-teal/40 bg-brand-teal/5" : "border-text-muted/20 hover:border-brand-teal/20 bg-bg-tertiary/30",
        ...
        <AnimatePresence mode="wait">
          {!file ? (
            ...
            >
              ...
              <div>
                <p className="font-syne font-bold text-lg">Select Delivery Manifest</p>
                <p className="text-text-slate text-xs mt-1">PDF, JPG, PNG or ZIP (Max 10MB)</p>
              </div>
            ...
            >
              <div className="flex items-center justify-between p-4 rounded-xl bg-bg-secondary border border-text-muted/10">
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[10px] font-mono text-text-slate uppercase">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                  </div>
                </div>
                {!uploading && (
                  <button 
              ...
              {!uploading && (
                <div className="flex gap-3">
                  ...
                  <Button className="flex-1" onClick={handleUpload}>Confirm Transmission</Button>
                </div>
              )}
            </motion.div>
          )}
      ...
      <div className="p-4 rounded-xl bg-brand-teal/5 border border-brand-teal/10 flex items-start gap-3">
        ...
        <p className="text-[11px] text-text-slate leading-relaxed uppercase tracking-wider text-left">
          Upon transmission, the file manifest is hashed and broadcasted to the SettleOne network. This action will trigger the verification protocol.
        </p>
      </div>
    </div>
  )
